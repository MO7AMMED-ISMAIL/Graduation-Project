...
<div class="row">
    <div class="col-lg-12">
        <div class="p-5">
            ...
            <form action="Tasks/update.php" method="post" class="user">
            <div class="form-group">
                    <input type="hidden" class="form-control form-control-user" name="token" value="<?=$_SESSION['token']?>">
                </div>
                <div class="form-group">
                    <input type="hidden" class="form-control form-control-user" name="id" value="<?=$taskID?>">
                </div>
                <div class="form-group">
                    <textarea class="form-control" name="content" rows="3" id="area" placeholder="Enter Content Post" style="border-radius: 15px;"><?=$SelTask['task_content']?></textarea>
                </div>
                <div class="form-group">
                <select class="form-control" style="height: 50px;border-radius: 15px;" name="email">
                    <option selected value="<?=$SelTask['task_to']?>">
                        <?=$SelTask['task_to']?>
                    </option>
                    <?php
