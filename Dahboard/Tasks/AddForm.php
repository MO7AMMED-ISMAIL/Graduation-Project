    ...
    <div class="col-lg-12">
        <div class="p-5">
            <div class="text-center">
                <h2 class="h4 text-gray-900 mb-4">Create New Task</h2>
            </div>
            <form action="Tasks/add.php" method="post" class="user">
            <div class="form-group">
                    <input type="hidden" class="form-control form-control-user" name="token" value="<?=$_SESSION['token']?>">
                </div>
                <div class="form-group">
                    <textarea class="form-control" name="content" rows="3" id="area" placeholder="Enter Content Task" style="border-radius: 15px;"></textarea>
                </div>
                <div class="form-group">
                    <select class="form-control" style="height: 50px;border-radius: 15px;" name="email">
                        <option selected>Send to user email</option>
                        <?php 
                            foreach($userEmail as $email){
                        ?>
                            <?php
                                if($email['user_email'] != $_SESSION['Admin_email'] && $email['user_role']==1){
                            ?>
